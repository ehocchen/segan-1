...
flags.DEFINE_string("test_dir", None, "dir of test wav (it won't train)")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if False:
            ...
            for dirname, dirs, files in os.walk(FLAGS.test_dir):
                for filename in files:
                    input_filepath = os.path.abspath(os.path.join(dirname, filename))
                    fm, wav_data = wavfile.read(input_filepath)
                    if fm != 16000:
                    ...
                    c_wave = c_wave / np.max(np.abs(c_wave)) * 32767
                    #c_wave = c_wave.astype(np.int16)
                    wavfile.write(os.path.join(FLAGS.save_clean_path, filename), 16e3, c_wave.astype(np.int16))
